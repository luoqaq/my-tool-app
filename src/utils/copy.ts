...
import { writeText, readText, onClipboardUpdate, hasText, hasImage, hasFiles, onFilesUpdate, onHTMLUpdate, onImageUpdate, onRTFUpdate, onTextUpdate, startListening, hasHTML, hasRTF, writeImageBase64 } from 'tauri-plugin-clipboard-api';
 ...
 * @param text 内容
 */
export async function copyToClipboard(content: string, type = CopyCatchType.text) {
 ...
 * 监听剪切板的变化
 */
export const useListenClipboardChange = (onCopyDataChange?: (copyData: CopyCatchItem) => void) => {
  ...
  const startWatch = useCallback(async () => {
    ...
    unlistenImageUpdate.current = await onImageUpdate((b64Str) => {
      console.log('b64Str', b64Str);
      // setBase64Image(b64Str);
      const now = Date.now();
      // 做1s内防抖
      if (b64Str === copyData?.content && lastClipboardTime + DebounceGap > now) {
        return;
      }
      const data = {
        id: `${now}_${Math.ceil(Math.random() * 100)}`,
...
export const useSaveClipboardToLocal = () => {
  ...
  useEffect(() => {
    if (!copyCatchList?.length) return;
    console.log('saveClipboardToLocal', copyCatchList);
    saveToLocal(LocalTextFIlePath, JSON.stringify(copyCatchList)).then(res => {
